...
$sql = "SELECT id, fname, lname, photo, kidschool, kidName, usern FROM personinfo where usern = '$vuser' ";
...
$result = $conn->query($sql);
    ...
    // output data of each row
       ...
       echo '<table id="table_id" class="display" ><thead><tr><td><u>RequestNo</u></td><td><u>who</u></td><td><u>from</u></td><td><u>To</u></td><td><u>date</u></td><td><u>-----</u></td><td><u>Action</u></td></tr></thead>';
       ...
       echo '<tbody>';
        ...
        if ($row["mornoon"] == 1 )  {
         ...
         echo '<td>  '.$row["homeAdd"].'</td>';   
          echo '<td>'.$row["kidschool"].'</td>';

          } 
    else {
     echo '<td>school'.$row["kidschool"].'</td>';
     echo '<td>'.$row["homeAdd"].'</td>';


   }

    echo '<td>'.$row['ridate'].'</td>';
    echo '<td><input type="button" value="direction" "</td>';
    echo '<td> <input type="checkbox" id="fixit" name="fixit[]" value="1" ></td></tr>';
...
if ($result->num_rows > 0) {
  ...
  echo '<table><tr><td><p><u>RequestNo</u></p></td><td><p><u>who</u></p></td><td><p><u>from/to</u></p></td><td><p><u>date</u></p></td><td><p><u>Action</u></p></td></tr>';
    ...
    if ($row["mornoon"] == 1 )  {
         echo '<td><p>To'.$row["kidschool"].'</p></td>';}
   else {
    echo '<td><p>From'.$row["kidschool"].'</p></td>';}
  }
  echo '</table>';
